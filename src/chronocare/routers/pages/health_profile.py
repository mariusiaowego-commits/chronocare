...
@router.get("/health-profile", response_class=HTMLResponse)
async def health_profile_page(
    request: Request,
    person_id: int | None = Query(None),
    db: AsyncSession = Depends(get_db),
):
    """Health profile overview page."""
    ...
    return templates.TemplateResponse(request, "health_profile/index.html", {
        "request": request,
        "persons": persons,
        "selected": selected,
